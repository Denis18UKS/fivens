...
public final class FifthClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.OPEN_COMPUTER, (client, handler, buf, responseSender) -> {
        ...
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.CUTSCENE_PAYLOAD, (client, handler, buf, responseSender) -> {
            String json = buf.readString(1_000_000);
            client.execute(() -> {
                try {
                    CutscenePlayback.start(GSON.fromJson(json, CutsceneDefinition.class));
                    client.setScreen(null);
                } catch (Exception ignored) {}
            });
        });
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.OPEN_LIFT_PANEL, (client, handler, buf, responseSender) -> {
            int id = buf.readVarInt(), mask = buf.readVarInt();
            boolean edit = buf.readBoolean();
            double x = buf.readDouble(), y = buf.readDouble(), z = buf.readDouble();
            float yaw = buf.readFloat();
            client.execute(() -> client.setScreen(new LiftPanelScreen(client.currentScreen, id, mask, edit, x, y, z, yaw)));
        });
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.VHS_PLAYBACK, (client, handler, buf, responseSender) -> {
            String json = buf.readString(1_000_000);
            int mode = buf.readVarInt();
            var pos = buf.readBlockPos();
            client.execute(() -> {
                try {
                    VhsPlayback.start(GSON.fromJson(json, CutsceneDefinition.class), mode, pos);
                } catch (Exception ignored) {}
            });
        });
        ...
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.CUTSCENE_EDIT_PAYLOAD, (client, handler, buf, responseSender) -> {
            ...
            client.execute(() -> {
                try {
                    var scene = GSON.fromJson(json, CutsceneDefinition.class);
                    client.setScreen(new CameraEditorScreen(client.currentScreen, scene));
                } catch (Exception ignored) {}
            });
        });
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.LIFT_TRAVEL_START, (client, handler, buf, responseSender) -> {
            int from = buf.readVarInt(), to = buf.readVarInt(), ticks = buf.readVarInt();
            client.execute(() -> LiftTravelOverlay.start(from, to, ticks));
        });
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.LIFT_TRAVEL_END, (client, handler, buf, responseSender) -> {
        ...
        ClientPlayNetworking.registerGlobalReceiver(FifthNetworking.ENTITY_EFFECT_SYNC, (client, handler, buf, responseSender) -> {
            ...
            client.execute(() -> EntityEffectRenderer.update(cfg));
        });
        ...
        UseEntityCallback.EVENT.register((player, world, hand, entity, hit) -> {
            ...
            if (world.isClient && player.getStackInHand(hand).isOf(FifthMod.ANIMATION_CONDITION_TOOL)) {
                MinecraftClient.getInstance().setScreen(new AnimationConditionScreen(MinecraftClient.getInstance().currentScreen, entity));
                return net.minecraft.util.ActionResult.SUCCESS;
            }
