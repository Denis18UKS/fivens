...
/** 2D 9-floor panel. Burned buttons are physically non-clickable outside edit mode. */
public final class LiftPanelScreen extends HorrorScreen {
    ...
    @Override public void render(DrawContext c,int mx,int my,float d){horrorBackground(c);c.drawCenteredTextWithShadow(textRenderer,edit?"Сожжённые кнопки недоступны игрокам. В редакторе их можно включать/выключать.":"Выбери этаж",width/2,safeTop(),0xFFD0B9AE);if(edit&&!status.isBlank())c.drawCenteredTextWithShadow(textRenderer,status,width/2,height-safeBottom()-12,0xFFD99090);super.render(c,mx,my,d);}
}
