...
public class CameraEditorScreen extends HorrorScreen {
    ...
    @Override protected void init() {
        ...
        int w = contentWidth(500), x = (width - w) / 2, top = safeTop(), gap = 6;
        ...
        int by = top + 57, bh = 21, half = (w - gap) / 2;
    ...
    private void addFrame() {
        ...
        int dur = 40; double fieldOfView = 70;
        try { dur = Integer.parseInt(duration.getText()); } catch (Exception ignored) {}
        try { fieldOfView = Double.parseDouble(fov.getText()); } catch (Exception ignored) {}
        dur = Math.max(1, Math.min(12000, dur));
        fieldOfView = Math.max(1.0, Math.min(179.0, fieldOfView));
        var player = client.player;
        var keyframe = new CutsceneDefinition.Keyframe(player.getX(), player.getEyeY(), player.getZ(), player.getYaw(), player.getPitch(), fieldOfView, dur);
    ...
    private boolean save() {
        ...
        scene.hideHud = hideHud; scene.lockInput = lock; scene.teleportPlayerAtEnd = teleportAtEnd;
        ...
        PacketByteBuf out = PacketByteBufs.create(); out.writeString(GSON.toJson(scene), 1_000_000);
        ClientPlayNetworking.send(FifthNetworking.SAVE_CUTSCENE, out); status = "Сцена сохранена: " + scene.id; return true;
    }

    private void play() {
        if (!save()) return;
        PacketByteBuf out = PacketByteBufs.create(); out.writeString(scene.id, 128);
        ClientPlayNetworking.send(FifthNetworking.PLAY_CUTSCENE, out); client.setScreen(null);
    }
    ...
    @Override public void render(DrawContext c, int mx, int my, float delta) {
        ...
        if (height > 245) c.drawCenteredTextWithShadow(textRenderer, "Кадров: " + scene.keyframes.size() + ". Последний кадр удерживается на свою длительность.", width / 2, infoY, 0xFFB4A49C);
