...
/** Camera/cutscene editor with per-world draft persistence and direct VHS creation. */
public class CameraEditorScreen extends HorrorScreen {
    ...
    @Override protected void init(){
        ...
        addDrawableChild(HorrorButton.builder(Text.literal("📼 Получить кассету"),b->cassette()).dimensions(x,by+step*4,w,bh).build());
    ...
    private void cassette(){if(!save())return;PacketByteBuf out=PacketByteBufs.create();out.writeString(scene.id,128);ClientPlayNetworking.send(FifthNetworking.CREATE_CASSETTE,out);status="Кассета с записью выдана: "+scene.id;}
