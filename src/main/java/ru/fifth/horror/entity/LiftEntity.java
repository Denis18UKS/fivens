...
public final class LiftEntity extends Entity implements GeoEntity {
    ...
    private int openFloorMask = 0x1FF; // floors 1..9
    ...
    public boolean canOpenOnFloor(int floor){int bit=1<<(Math.max(1,Math.min(9,floor))-1);return (openFloorMask&bit)!=0;}
    public void setOpenOnFloor(int floor,boolean open){int bit=1<<(Math.max(1,Math.min(9,floor))-1);openFloorMask=open?(openFloorMask|bit):(openFloorMask&~bit);}
    ...
    @Override protected void readCustomDataFromNbt(NbtCompound nbt){
        ...
        currentFloor=Math.max(1,nbt.getInt("FivenFloor")); targetFloor=Math.max(1,nbt.getInt("FivenTargetFloor"));
        openFloorMask=nbt.contains("FivenOpenMask")?nbt.getInt("FivenOpenMask"):0x1FF;
        stageOrigin=nbt.contains("FivenStageOrigin")?BlockPos.fromLong(nbt.getLong("FivenStageOrigin")):null;
    }
    @Override protected void writeCustomDataToNbt(NbtCompound nbt){
        nbt.putString("FivenLiftId",liftId);nbt.putInt("FivenFloor",currentFloor);nbt.putInt("FivenTargetFloor",targetFloor);nbt.putInt("FivenOpenMask",openFloorMask);
