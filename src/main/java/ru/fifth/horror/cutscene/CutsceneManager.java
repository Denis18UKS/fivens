...
public final class CutsceneManager {
    ...
    public static void play(MinecraftServer server,String id){CutsceneDefinition c=load(server,id);if(c==null)return;String json=GSON.toJson(c);for(ServerPlayerEntity p: PlayerLookup.all(server)){PacketByteBuf buf=PacketByteBufs.create();buf.writeString(json,1_000_000);ServerPlayNetworking.send(p,FifthNetworking.CUTSCENE_PAYLOAD,buf);}}
